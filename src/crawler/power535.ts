import { RequestPower535, LotteryResult, CrawlOptions, Task } from '../types/index.js';
import { DEFAULT_ORENDER_INFO } from '../config/request.js';
import { power535Config } from '../config/products.js';
...
export class Power535Crawler {
  private readonly url = 'https://vietlott.vn/ajaxpro/Vietlott.PlugIn.WebParts.Game535CompareWebPart,Vietlott.PlugIn.WebParts.ashx';
  private readonly config = power535Config;

  private createRequestBody(pageIndex: number): RequestPower535 {
    return {
      ORenderInfo: DEFAULT_ORENDER_INFO,
      Key: 'd0ea794f',
      GameDrawId: '',
      ArrayNumbers: Array(5).fill(null).map(() => Array(35).fill('')),
      CheckMulti: false,
      PageIndex: pageIndex,
    };
  }
  ...
  private parseResult(responseData: any, pageIndex: number): LotteryResult[] {
    ...
    try {
      ...
      // Parse each table row - similar to Power 645
      ...
      while ((match = rowRegex.exec(html)) !== null) {
        ...
        // Extract lottery numbers from <span class="bong_tron"> tags
  ...
  async crawl(options: CrawlOptions = {}): Promise<void> {
    ...
    logger.info(`Starting crawl for Power 5/35`);
