import { RequestMax3DPro, LotteryResult, CrawlOptions, Task } from '../types/index.js';
import { DEFAULT_ORENDER_INFO } from '../config/request.js';
import { max3dProConfig } from '../config/products.js';
...
export class Max3DProCrawler {
  private readonly url = 'https://vietlott.vn/ajaxpro/Vietlott.PlugIn.WebParts.GameMax3DProCompareWebPart,Vietlott.PlugIn.WebParts.ashx';
  private readonly config = max3dProConfig;

  private createRequestBody(pageIndex: number): RequestMax3DPro {
    return {
      CheckMulti: 0,
      GameDrawId: '',
      GameId: '7',
  ...
  private parseResult(responseData: any, pageIndex: number): LotteryResult[] {
    ...
    try {
      ...
      // Parse table rows - same logic as Max3D
      ...
      for (const rowHtml of rows) {
        ...
        // Define prizes and their counts (same as Max3D)
        const prizes = [
  ...
  async crawl(options: CrawlOptions = {}): Promise<void> {
    ...
    logger.info(`Starting crawl for Max3D Pro`);
