import { RequestPower645, LotteryResult, CrawlOptions, Task } from '../types/index.js';
import { DEFAULT_ORENDER_INFO } from '../config/request.js';
import { power645Config } from '../config/products.js';
...
export class Power645Crawler {
  private readonly url = 'https://vietlott.vn/ajaxpro/Vietlott.PlugIn.WebParts.Game645CompareWebPart,Vietlott.PlugIn.WebParts.ashx';
  private readonly config = power645Config;

  private createRequestBody(pageIndex: number): RequestPower645 {
    return {
      ORenderInfo: DEFAULT_ORENDER_INFO,
      Key: '785cdae0',
      GameDrawId: '',
      ArrayNumbers: Array(6).fill(null).map(() => Array(18).fill('')),
      CheckMulti: false,
      PageIndex: pageIndex,
    };
  }
  ...
  private parseResult(responseData: any, pageIndex: number): LotteryResult[] {
    ...
    try {
      ...
      // Match pattern: <td>DD/MM/YYYY</td> followed by <td><a ...>ID</a></td> followed by lottery numbers
      const rowRegex = /<tr>\s*<td>(\d{2}\/\d{2}\/\d{4})<\/td>\s*<td><a[^>]*>(\d+)<\/a><\/td>\s*<td>([\s\S]*?)<\/td>\s*<\/tr>/g;
      ...
      while ((match = rowRegex.exec(html)) !== null) {
        ...
        // Extract lottery numbers from <span class="bong_tron">XX</span> tags
        const numberRegex = /<span class="bong_tron[^"]*">(\d+)<\/span>/g;
        ...
        // Convert date from DD/MM/YYYY to YYYY-MM-DD for consistency
  ...
  async crawl(options: CrawlOptions = {}): Promise<void> {
    ...
    logger.info(`Starting crawl for Power 6/45`);
